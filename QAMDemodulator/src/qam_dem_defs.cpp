...
void QAMDEM::mixer( cdata_t din, cdata_t *dout_mix, cphase_t ph_offset ) {
    cdata_t t_dout;
...
static ap_fixed<16,1> phase_angle; 
ap_fixed<32,2> diff;
cphase_t       t_ph;
...
// Floating point, give a try ( la vida es un riesgo, carnal );
float f_angle;
float f_cos,f_sin;

f_angle = phase_angle;  
f_angle = 3.1416 * f_angle;  
f_cos   = cos(f_angle);
f_sin   = sin(f_angle);
...
void QAMDEM::mounstrito (
  ...
  QAMDEM::mixer( din, &t_dout_mix, ph_in );
